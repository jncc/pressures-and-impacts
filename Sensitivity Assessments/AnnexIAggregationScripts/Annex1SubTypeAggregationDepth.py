...
def main():
    ...
    L5_orig = pd.DataFrame(maresa_annex_merge.loc[maresa_annex_merge['EUNIS_Level'].isin(['5'])])
    ...
    L56_diff.columns = [
        'JNCC_Code_x', 'Annex I Habitat', 'Annex I sub-type', 'Depth zone', 'Classification level_x',
    ...
    # aggregated
    L56_diff['FeatureSubFeatureDepth'] = L56_diff.apply(lambda row: together(row), axis=1)
    ...
    # .apply() method
    L5_agg = L56_diff.groupby(['Level_5', 'Pressure', 'FeatureSubFeatureDepth']
    ...
    L5_agg.columns = ['Level_5', 'Pressure', 'FeatureSubFeatureDepth', 'Sensitivity']
    ...
    L5_orig_sub['FeatureSubFeatureDepth'] = L5_orig_sub.apply(lambda row: together(row), axis=1)
    ...
    L5_all = L5_all[[
        'Pressure', 'FeatureSubFeatureDepth', 'Level_4', 'Level_5', 'Sensitivity', 'L5_FinalSensitivity',
    ...
    annex_aggregation = annex_aggregation[
        [
            'Pressure', 'FeatureSubFeatureDepth', 'Level_5', 'L5_FinalSensitivity', 'L5_AssessedCount',
    ...
    eunis_only_notSNH = annex_aggregation.loc[annex_aggregation['FeatureSubFeatureDepth'].isin(['nan - nan'])]
    ...
    # Reset columns within L5_agg DF
    feature_sub_aggregation.columns = ['Pressure', 'FeatureSubFeatureDepth', 'Sensitivity']
    ...
    feature_sub_aggregation = feature_sub_aggregation.loc[
        ~feature_sub_aggregation['FeatureSubFeatureDepth'].isin(eunis_no_SNH_feature)
    ]
    ...
    feature_sub_aggregation['Depth zone'] = feature_sub_aggregation.apply(
    ...
    feature_sub_aggregation = feature_sub_aggregation[[
        ...
        'Depth zone', 'FinalSensitivity', 'AssessedCount', 'UnassessedCount'
    ]]
    ...
    filename = "FeatureLevelAggregationDepth_" + (time.strftime("%Y%m%d") + ".csv")
