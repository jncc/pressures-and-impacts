...
# Title: Annex I SubType Aggregation FeAST
...
def main():
    ...
    annex1 = pd.read_excel(r"\\jncc-corpfile\JNCC Corporate Data\Marine\Evidence\PressuresImpacts\6. Sensitivity\FeAST\Annex1_FeAST\Trial_AnnexI_sub-types\AnnexI_sub_types_all_v5.xlsx", 'Sheet1', dtype=str)
    ...
    # aggregated
    maresa_annex_merge['FeatureSubFeature'] = maresa_annex_merge.apply(lambda row: together(row), axis=1)

    # Extract all level 6 data only and assign to object oriented variable to be aggregated to level 5
    original_L6_data = pd.DataFrame(maresa_annex_merge.loc[maresa_annex_merge['EUNIS_Level'].isin(['6'])])

    # Group all L6 data by Sensitivity values
    L6_processed = original_L6_data.groupby(['Level_5', 'Pressure', 'FeatureSubFeature'
    ...
    L6_processed.columns = ['Level_6', 'Pressure', 'FeatureSubFeature', 'Sensitivity']
    ...
    # .apply() method
    aggregated_L6_to_L5 = L6_processed.groupby(['Level_5', 'Pressure', 'FeatureSubFeature']
                                               )['Sensitivity'].apply(lambda x: ', '.join(x))
    ...
    aggregated_L6_to_L5.columns = ['Level_5', 'Pressure', 'FeatureSubFeature', 'Sensitivity']

    # Extract all original level 5 data and assign to object oriented variable
    original_L5_data = pd.DataFrame(maresa_annex_merge.loc[maresa_annex_merge['EUNIS_Level'].isin(['5'])])
    ...
    original_L5_without_L6_DF = original_L5_without_L6_DF[['Level_5', 'Pressure', 'FeatureSubFeature', 'Sensitivity']]
    ...
    aggregated_L6_to_L5['Level_4'] = aggregated_L6_to_L5.apply(lambda df: pd.Series(column4(df)), axis=1)
    ...
    # Create DataFrame for master DataFrame at end of script - NOT NECESSRY??
    # annex_aggregation = pd.merge(L5_export, L6_export, how='outer', on=['Pressure', 'FeatureSubFeature'])

    # Rename and reorder columns within annex_aggregation
    annex_aggregation = L5_all[
        [
            'Pressure', 'FeatureSubFeature', 'Level_5', 'Sensitivity', 'L5_FinalSensitivity', 'L5_AssessedCount',
