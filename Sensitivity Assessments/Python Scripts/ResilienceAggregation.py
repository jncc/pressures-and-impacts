...
# This script allows the user to aggregate resilience data across varying tiers of EUNIS hierarchies. The main purpose
...
fact_tbl['Resilience'].fillna(value='', inplace=True)

# Remove unwanted special characters from data by passing the fact_tbl to the unwanted_char() function
fact_tbl = unwanted_char(fact_tbl, 'Resilience')

# Create individual EUNIS level columns in fact_tbl using a lambda function and apply() method on 'EUNIS_Code' column
# on DF.
...
# this will allow the end user to identify which resilience scores are relevant to lower EUNIS levels when an
...
L56_diff = L56_diff.drop(['ID_x_y', 'SubregionName_y', 'RegionName_y', 'BiotopePresence_y', 'EUNIS_Code_y',
                         'HabitatName_y', 'Gaps_y', 'ID_y_y', 'Name_y', 'NE_Code_y', 'Resistance_y', 'ResistanceQoE_y',
                          ...
                          'resilienceDoE_y', 'Sensitivity_y', 'SensitivityQoE_y', 'SensitivityAoE_y',
                          'SensitivityDoE_y', 'url_y', 'Level_1_y', 'Level_2_y', 'Level_3_y', 'Level_4_y', 'Level_6_y',
                          'EUNIS_Level_y', '_merge'], axis=1, inplace=False)
...
# Group data by Level_5, Pressure, SubregionName and apply resilience values to list using lambdas function and .apply()
# method
L5_agg = L56_diff.groupby(['Level_5', 'Pressure', 'SubregionName_x']
                          )['Resilience_x'].apply(lambda x: ', '.join(x))
...
L5_agg.columns = ['Level_5', 'Pressure', 'SubregionName', 'Resilience']
...
L5_agg[['High', 'Medium', 'Low', 'Not sensitive', 'Not relevant', 'No evidence', 'Not assessed',
        'Unknown']] = L5_agg.apply(lambda df: pd.Series(counter(df['Resilience'])), axis=1)
...
# Reassign L5_agg DataFrame to L5_sens for resilience aggregation
L5_res = L5_agg
...
L5_res['L5_Resilience'] = L5_res.apply(lambda df: create_resilience(df), axis=1)

# Use lambda function to apply final_resilience() function to each row within the DataFrame
L5_res['L5_FinalResilience'] = L5_res.apply(lambda df: final_resilience(df), axis=1)
...
L5_agg_res = L5_agg_res.drop(['Resilience', 'Unknown', 'High', 'Medium', 'Low', 'Not sensitive', 'Not relevant',
...
L5_agg_res.rename(columns={'L5_Resilience': 'Resilience'}, inplace=True)
...
L5_orig_sub = L5_orig_sub.drop(['ID_x', 'RegionName', 'BiotopePresence', 'EUNIS_Code', 'HabitatName', 'Gaps', 'ID_y',
                                'Name',
                                'NE_Code', 'Resistance', 'ResistanceQoE', 'ResistanceAoE', 'ResistanceDoE',
...
L5_orig_sub[['High', 'Medium', 'Low', 'Not sensitive', 'Not relevant', 'No evidence', 'Not assessed',
             'Unknown']] = L5_orig_sub.apply(lambda df: pd.Series(counter(df['Resilience'])), axis=1)
...
# Use lambda function to apply final_resilience() function to each row within the DataFrame
L5_orig_sub['L5_FinalResilience'] = L5_orig_sub.apply(lambda df: final_resilience(df), axis=1)
...
L5_all = L5_all[['Pressure', 'SubregionName', 'Level_4', 'Level_5', 'Resilience', 'L5_FinalResilience',
...
# Group data by Level_4, Pressure, SubregionName and apply resilience values to list using lambdas function and .apply()
# method
...
L4_agg[['High', 'Medium', 'Low', 'Not sensitive', 'Not relevant', 'No evidence', 'Not assessed',
        'Unknown']] = L4_agg.apply(lambda df: pd.Series(counter(df['Resilience'])), axis=1)
...
# Reassign L4_agg DataFrame to L4_sens for resilience aggregation
L4_res = L4_agg
...
L4_res = L4_res[['Level_3', 'Pressure', 'SubregionName', 'Level_4', 'Resilience', 'L4_Resilience',
                   'L4_FinalResilience', 'L4_AssessedCount', 'L4_UnassessedCount', 'L4_AssessmentConfidence']]
...
# Group data by Level_3, Pressure, SubregionName and apply resilience values to list using lambdas function and .apply()
# method
...
# Reassign L3_agg DataFrame to L3_sens for resilience aggregation
L3_res = L3_agg
...
# Group data by Level_2, Pressure, SubregionName and apply resilience values to list using lambdas function and .apply()
# method
...
# Reassign L2_agg DataFrame to L2_sens for resilience aggregation
L2_res = L2_agg
...
L2_res = L2_res.drop(['Resilience', 'Unknown', 'High', 'Medium', 'Low', 'Not sensitive', 'Not relevant',
                      'No evidence', 'Not assessed', 'Count_High', 'Count_Medium', 'Count_Low',
                      'Count_NotSensitive', 'Count_NotRel', 'Count_NoEvidence',
                      'Count_NotAssessed', 'Count_Unknown'], axis=1, inplace=False)
...
MasterFrame = MasterFrame[['Pressure', 'SubregionName', 'Level_2', 'L2_FinalResilience', 'L2_AssessedCount',
                           'L2_UnassessedCount', 'L2_AssessmentConfidence', 'L2_ConfidenceCategory', 'Level_3',
                           'L3_FinalResilience', 'L3_AssessedCount', 'L3_UnassessedCount', 'L3_AssessmentConfidence',
                           'L3_ConfidenceCategory', 'Level_4', 'L4_FinalResilience', 'L4_AssessedCount',
                           'L4_UnassessedCount', 'L4_AssessmentConfidence', 'L4_ConfidenceCategory', 'Level_5',
                           'L5_FinalResilience', 'L5_AssessedCount', 'L5_UnassessedCount', 'L5_AssessmentConfidence',
