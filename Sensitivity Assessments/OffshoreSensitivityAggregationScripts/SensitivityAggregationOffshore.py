...
def main():
    ...
    # # Function Title: unknown_check
    # def unknown_check(row):
    #     """
    #     User defined function to assess if the EUNIS level 5 sensitivity value of the target DF is 'Unknown' and check
    #     if there is a correlating level 6 assessment completed (which is not unknown). If there is, return a value to be
    #     assigned into a new column when running this function. This data is subsequently used to delete rows based on a
    #     specific value."""
    #     # Pull out EUNIS L5 data from target DF row
    #     ecode = row['Level_5']
    #     # Pull out the EUNIS codes present within the L56_diff DF
    #     l6_data = L56_diff.loc[L56_diff['EUNIS_Code_x'].apply(len) >= 6]
    #     # Drop L6 data from the l6_code DF which is 'Unknown' (only interested in the l6 with real assessed values
    #     l6_data = l6_data[l6_data['Sensitivity_x'] != 'Unknown']
    #
    #     # Check if the assessment value is an 'Unknown'
    #     if row['Sensitivity'] == 'Unknown':
    #         # Check if the correlating EUNIS code is present within the EUNIS codes in the L56_diff DF
    #         if any([ecode in x for x in l6_data['EUNIS_Code_x']]):
    #             return 'To be deleted'
    #         else:
    #             return 'To be retained'
    ...
    # Drop unwanted columns from L6_export DataFrame
    # L6_export = L6_export.drop(['L6_Sensitivity', 'Sensitivity'], axis=1, inplace=False)
    ...
    # Define folder file path to be saved into
    outpath = r"J:\GISprojects\Marine\Sensitivity\MarESA aggregation\MarESA_AggregationOutputs_Main\MarESAAggregationOutputs\Sensitivity"
    # Define file name to save, categorised by date
    ...
    print("The 'SensitivityAggregationOffshore' script took " + str(round(elapsed / 60, 1)) + ' minutes to run and complete.' + '\n' +
          'This has been saved as an output at the following filepath: J:\GISprojects\Marine\Sensitivity\MarESA aggregation\MarESA_AggregationOutputs_Main\MarESAAggregationOutputs\Sensitivity')
